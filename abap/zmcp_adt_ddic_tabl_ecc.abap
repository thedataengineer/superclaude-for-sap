...
* ZMCP_ADT_DDIC_TABL — ECC 7.40 DDIC table fallback (ECC-only).
* Mirrors the DOMA FM pattern:
*   RS_CORR_INSERT → DDIF_TABL_PUT → TR_RECORD_OBJ_CHANGE_TO_REQ
*   → WB_TREE_ACTUALIZE (SE80 object-tree refresh)
* See zmcp_adt_ddic_doma_ecc.abap header for details.
*
* Note: DDIF_TABL_ACTIVATE may return rc=4 (warnings) when DD03P rows
* are passed with only FIELDNAME + ROLLNAME. The table activates and
* is functionally usable; warnings typically relate to missing field
* attributes (INTTYPE/DATATYPE/LENG/DECIMALS) that are resolved from
* the referenced data element at activation time. Populate the full
* DD03P if a clean rc=0 activation is required.
      ...
      CASE iv_action.
        ...
        WHEN 'CREATE' OR 'UPDATE'.
          ...
          IF iv_devclass IS NOT INITIAL AND iv_devclass <> '$TMP'.
            PERFORM register_corr_tabl USING lv_name iv_devclass iv_transport
          ...
          /ui2/cl_json=>deserialize(
            ...
            CHANGING  data = ls_payload_t ).

          ls_dd02v = ls_payload_t-dd02v.
          lt_dd03p = ls_payload_t-dd03p.
          ls_dd02v-tabname = lv_name.

          LOOP AT lt_dd03p ASSIGNING FIELD-SYMBOL(<ls_fld>).
            <ls_fld>-tabname = lv_name.
          ENDLOOP.
          ...
          ev_subrc = sy-subrc.
          IF sy-subrc <> 0.
            ev_message = |DDIF_TABL_PUT failed (sy-subrc={ sy-subrc })|.
            RETURN.
          ENDIF.

          IF iv_devclass IS NOT INITIAL AND iv_devclass <> '$TMP' AND iv_transport IS NOT INITIAL.
            PERFORM record_to_req_tabl USING lv_name iv_devclass iv_transport
                                     CHANGING ev_subrc ev_message.
            IF ev_subrc <> 0.
              RETURN.
            ENDIF.
          ...
          IF iv_devclass IS NOT INITIAL AND iv_devclass <> '$TMP'.
            PERFORM refresh_wb_tree_tabl USING iv_devclass.
          ENDIF.

          ev_subrc   = 0.
          ev_message = |Table { lv_name } staged inactive; call ZMCP_ADT_DDIC_ACTIVATE to activate|.
        ...
        WHEN 'DELETE'.
          ...
          ev_subrc = sy-subrc.
          IF sy-subrc <> 0.
            ev_message = |RS_DD_DELETE_OBJ failed for TABL { lv_name } (sy-subrc={ sy-subrc })|.
          ELSE.
            IF iv_devclass IS NOT INITIAL AND iv_devclass <> '$TMP'.
              PERFORM refresh_wb_tree_tabl USING iv_devclass.
            ENDIF.
            ev_message = |Table { lv_name } deleted|.
...
FORM record_to_req_tabl
  ...
  TRY.
      ...
      ev_subrc = 0.
      ev_message = |Recorded { lv_cnt } TADIR entries for TABL { iv_name } -> { iv_transport }|.
...
FORM refresh_wb_tree_tabl USING iv_devclass TYPE string.
