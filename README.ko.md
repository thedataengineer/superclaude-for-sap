...
이후 Claude Code를 재시작(또는 `/plugin` → reload)하여 스킬·에이전트·훅을 반영시킵니다. 확인:
...
| 2 | **SAP 시스템 버전** | `S4` (S/4HANA — BP, MATDOC, ACDOCA, Fiori, CDS) 또는 `ECC` (ECC 6.0 — XK01/XD01, MKPF/MSEG, BKPF/BSEG) 선택. 이후 **ABAP Release** 입력 (예: `750`, `756`, `758`). 에이전트가 참조할 SPRO 테이블 / BAPI / TCode 와 사용 가능한 ABAP 문법 범위를 결정 |
...
- **v0.1.x** (현재) — 24 에이전트, 17 스킬, 13 모듈 컨피그, 공통 `common/` 규약, SPRO 로컬 캐시, 데이터 추출 차단 리스트(**L1–L4 모두 출시**; L4는 `abap-mcp-adt-powerup`의 `SC4SAP_POLICY=on` 환경변수로 opt-in), Cloud ABAP 인식
