...
Prism for SAP verwandelt Claude Code in einen Full-Stack-SAP-Entwicklungsassistenten. Über den [MCP ABAP ADT Server](https://github.com/abap-mcp-adt-powerup) (150+ Tools) verbindet es sich mit dem SAP-System und erstellt, liest, aktualisiert und löscht ABAP-Objekte direkt — Klassen, Funktionsbausteine, Reports, CDS-Views, Dynpro, GUI-Status und mehr.

## Kernfunktionen

| Funktion | Beschreibung |
|----------|--------------|
| 🔌 **Auto-MCP-Installation** | `abap-mcp-adt-powerup` wird während `/prism:setup` automatisch installiert, konfiguriert und verbindungsgetestet. Keine manuelle MCP-Verdrahtung. |
| 🌐 **Multi-Environment-Profile (Dev / QA / Prod)** | Mehrere SAP-Systeme pro Unternehmen registrieren (z.B. `KR-DEV`, `KR-QA`, `KR-PRD`, `US-DEV`) und via `/prism:sap-option` in der laufenden Sitzung umschalten. **QA- und Prod-Profile sind automatisch schreibgeschützt**: Eine 2-Schicht-Absicherung (PreToolUse-Hook + MCP-Server-Guard) blockiert `Create*/Update*/Delete*`, `CreateTransport` und Laufzeit-Code-Ausführung — das Umgehen des Hooks umgeht die Durchsetzung nicht. Passwörter werden im **OS-Schlüsselbund** (`@napi-rs/keyring` — Windows Credential Manager / macOS Keychain / libsecret) gespeichert, sodass `.prism/` niemals Secrets nach git leakt. Artefakte (Specs, CBO-Kataloge, Audits) sind pro Profil isoliert; schreibgeschützte Cross-View erlaubt QA-Sitzungen, Dev-erzeugte Specs zu inspizieren, ohne sie zu kontaminieren. |
...
| 🧪 **Code-Analyse** | `/prism:analyze-code` — dedizierter statischer Review-Pass (`sap-code-reviewer`): Clean ABAP, Performance, Security, SAP-Standard-Compliance. Nach Schweregrad sortierte Befunde mit konkreten Korrekturvorschlägen. |
| 🔀 **Programme vergleichen** | `/prism:compare-programs` — Business-orientierter Seite-an-Seite-Vergleich von 2–5 ABAP-Programmen, die dasselbe Szenario teilen, aber nach Modul (MM/CO), Land (KR/EU) oder Persona (Controller/Lager) aufgeteilt sind. Berater-gerechter Markdown-Bericht über 10 konfigurierbare Dimensionen. |
