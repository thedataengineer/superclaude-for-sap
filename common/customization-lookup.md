...
- If `.prism/customizations/{MODULE}/enhancements.json` exists:
...
- If `.prism/customizations/{MODULE}/extensions.json` exists:
...
- **Add a callout** telling the user their choice: "No customization inventory is present. Run `/prism:setup customizations {MODULE}` to check whether this BAdI already has a Z implementation before I create a new one."
...
3. Warn about token cost and offer the alternative of running `/prism:setup customizations`
...
about to recommend / critique / extend a standard SAP object
        │
        ▼
  .prism/customizations/{MODULE}/*.json present?
...
- The cache is populated by `/prism:setup customizations` (optional wizard step 11b)
- If missing, you MAY recommend `/prism:setup customizations` after the current task — but do not block on it
...
1. Local Customization Cache (`.prism/customizations/{MODULE}/{enhancements,extensions}.json`) — **priority 1 for any extension/enhancement recommendation**
