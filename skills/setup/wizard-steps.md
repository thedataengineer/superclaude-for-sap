...
> **Multi-profile note (0.6.0+)**: profile definitions live in `~/.prism/profiles/<alias>/{sap.env, config.json}`; the project folder carries only `<project>/.prism/active-profile.txt` plus `work/<alias>/` artifacts. The single-file `<project>/.prism/sap.env` exists only as a legacy state that Step 0 below migrates away. See [`../../docs/multi-profile-design.md`](../../docs/multi-profile-design.md) and [`../../docs/multi-profile-setup-gap.md`](../../docs/multi-profile-setup-gap.md).
...
- `needsMigration=true` → route to **[`../sap-option/migration.md`](../sap-option/migration.md)**. Collect `alias` + `tier`, run `sap-profile-cli.mjs migrate`, then DELETE `<project>/.prism/config.json` (decision §4.3 of the setup gap plan — engagement state is migrated into the new profile's config first). Resume at Step 5.
...
Clone (`github.com/abap-mcp-adt-powerup.git`) and build the external MCP server into the **plugin root's** `vendor/abap-mcp-adt/` folder (typically `~/.claude/plugins/marketplaces/prism/vendor/abap-mcp-adt/`), **NOT** the user's project directory.
...
- **⚠️ Path resolution (MANDATORY)**: the install target must be the plugin root, not the current working directory. Resolve the plugin root **dynamically** — never hardcode the path. This skill file lives at `<PLUGIN_ROOT>/skills/setup/SKILL.md`, so:
  ...
  - On Windows this will usually be `C:\Users\<user>\.claude\plugins\marketplaces\prism`, but **derive it at runtime**, do not assume
...
Full procedure: **[`wizard-step-04-profile-creation.md`](wizard-step-04-profile-creation.md)**. Collects `alias` + `SAP_TIER` + connection fields, then delegates write to `sap-profile-cli.mjs add` (profile env in `~/.prism/profiles/<alias>/`, password in OS keychain, active pointer in `<project>/.prism/active-profile.txt`). NO file is written under `<project>/.prism/sap.env` or `<project>/.prism/config.json`.
...
Parse the `GetSession` response for System ID (SID), MANDT (client), BNAME (user), SPRAS (language). Display them for the user to confirm, then **write them into the active profile's `~/.prism/profiles/<alias>/config.json` → `systemInfo`** (NOT the project folder).
...
- Re-run this step from `/prism:sap-option` or by re-invoking `/prism:setup` whenever the user connects to a different system. Each alias has its own `config.json` — data never crosses profiles.
...
**Tier gate**: Step 9 runs ONLY when the active profile's `SAP_TIER=DEV`. On QA/PRD, it REFUSES and prints CTS import guidance (decision §4.2). DEV installs are deduped by `SAP_URL + SAP_CLIENT` — sibling DEV profiles on the same system reuse the install; sentinel lives at `~/.prism/profiles/<alias>/.abap-utils-installed`.
...
Write the remaining plugin-side fields into `~/.prism/profiles/<alias>/config.json`:
...
The project folder NEVER has a `.prism/config.json` in multi-profile mode (decision §4.3). Only `active-profile.txt` + `work/<alias>/` artifacts live there.

Keep the profile's `sap.env` and `config.json` in sync: `sapVersion` / `abapRelease` / `industry` / `activeModules` are duplicated on purpose — the MCP server reads the env; plugin-side components (HUD, PreToolUse hook, skills, agents) read the config JSON. `/prism:sap-option edit` maintains both atomically.

## Steps 11 & 11b — Optional Extraction (SPRO + Customizations)

Full procedure: **[`wizard-step-11-optional-extraction.md`](wizard-step-11-optional-extraction.md)**. Both prompts optional. Artifacts land under `<project>/.prism/work/<activeAlias>/` per [`../../common/multi-profile-artifact-resolution.md`](../../common/multi-profile-artifact-resolution.md). Execution detail: [`spro-auto-generation.md`](spro-auto-generation.md) and [`customization-auto-generation.md`](customization-auto-generation.md).
...
- If no `statusLine` exists → writes the prism HUD entry.
- If a prism entry already exists → refreshes it.
- If a **non-prism** `statusLine` exists → leaves it alone and prints a warning; re-run with `--force` only if the user confirms they want to overwrite their custom status line.

After success, tell the user: "Restart Claude Code to see the HUD render below the input box." See `hud-statusline.md` for the full spec. The HUD reads `<project>/.prism/active-profile.txt` + the profile env to render `{alias} [{tier}] {🔒 if readonly}`.
