...
> Claude Code agent teams를 prism의 멀티 전문가 협의 워크플로에 적용하기 위한 설계 문서. 상태: **draft**, ask-consultant 프로토타입 착수 대기.
...
- Members = `Agent(..., team_name="...")`로 스폰되는 prism 에이전트.
...
**적용 스킬**:
- `/prism:ask-consultant` — 2개 이상 모듈 매칭 AND (주제가 경계를 넘거나 답변이 갈릴 때).
- `/prism:create-program` Phase 1A (모듈 인터뷰) — 활성 모듈 ≥ 2.
- `/prism:create-program` Phase 2 (플래닝) — 스펙이 크로스모듈 접점 선언.
- `/prism:compare-programs` Step 4b — 샘플셋이 2개 이상 모듈 걸침.
...
**적용 스킬**:
- `/prism:create-program` Phase 4 (구현) — executor가 여러 모듈에 걸친 코드 작성.
- `/prism:analyze-code` — reviewer가 14 차원 리뷰 중 business-alignment 차원(§ 1-2 업무 타당성, § 13 크로스모듈 사이드이펙트)에 컨설턴트 라이브 인풋.
...
**적용 스킬**:
- `/prism:analyze-symptom` — 덤프 / 증상이 Z/Y 객체를 거치며 모듈 컨텍스트가 얽힐 때 (순수 커널 이슈는 제외).
...
**적용 스킬**:
- `/prism:create-program` Phase 1A ↔ 1B 브릿지 — 현재는 순차(1A 모듈 인터뷰, 1B 프로그램 구조 인터뷰). 팀화하면 컨설턴트가 "이 요건이면 ALV보다 CDS+Fiori가 맞다"를 턴 내 즉시 제기 가능.
...
Round 2 (CHALLENGE):
  FI → MM: "MIGO 선행 → MIRO는 GR/IR 클리어링 필요; .prism/config.json의
...
- `/prism:create-program` Phase 4 (in-loop 검증).
- `/prism:analyze-code` (§ 1-2 + § 13 차원).

### Phase 6 — Type C 확장 (Incident Triage)
- `/prism:analyze-symptom`.

### Phase 7 — Type D 확장 (Interview Synthesis)
- `/prism:create-program` Phase 1A ↔ 1B 브릿지.
