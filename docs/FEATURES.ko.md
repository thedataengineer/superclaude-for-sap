...
| 스킬 | 설명 |
|------|------|
| `prism:setup` | 플러그인 설정 — MCP 서버 자동 설치, SPRO 구성 생성, 블록리스트 훅 설치 |
| `prism:mcp-setup` | 독립적인 MCP ABAP ADT 서버 설치/재설정 가이드 |
| `prism:sap-option` | `.prism/sap.env` 보기/편집 (자격증명, RFC 백엔드, 블록리스트, 활성 모듈) |
| `prism:sap-doctor` | 플러그인 + MCP + SAP 진단 (6 레이어) |
| `prism:create-object` | ABAP 객체 생성 (하이브리드 모드 — 전송+패키지 확인, 생성, 활성화) |
| `prism:create-program` | 풀 ABAP 프로그램 파이프라인 — Main+Include, OOP/Procedural, ALV, Dynpro, Text Elements, ABAP Unit |
| `prism:program-to-spec` | ABAP 프로그램을 Functional/Technical Spec으로 역공학 (Markdown / Excel) |
| `prism:compare-programs` | 동일 시나리오를 모듈/국가/페르소나로 분화한 2~5개 ABAP 프로그램을 비교 → 컨설턴트용 Markdown 리포트 |
| `prism:analyze-code` | ABAP 코드 분석 (Clean ABAP / 성능 / 보안) |
| `prism:analyze-cbo-obj` | CBO 인벤토리 스캐너 + 교차 모듈 갭 분석 |
| `prism:analyze-symptom` | SAP 운영 에러/증상 단계별 분석 (덤프, 로그, SAP Note 후보) |
| `prism:ask-consultant` | 모듈 컨설턴트 에이전트(SD/MM/FI/CO/PP/PS/PM/QM/TR/HCM/WM/TM/BW/Ariba/BC)에 직접 질의. 읽기 전용 — 설정된 SAP 환경에 맞추어 답변. |
| `prism:trust-session` | INTERNAL-ONLY — 세션 전체 MCP 권한 부트스트랩 |
| `prism:deep-interview` | 구현 전 Socratic 요구사항 수집 |
| `prism:team` | 조정된 병렬 에이전트 실행 (네이티브 Claude Code teams) |
| `prism:release` | CTS 전송 릴리즈 워크플로우 |
...
```
흐름: `GetPackageTree` → 카테고리별 walk → 빈도 휴리스틱 → 교차 모듈 갭 체크 → `.prism/cbo/<MODULE>/<PACKAGE>/inventory.json`.
...
### `/prism:sap-option`
`.prism/sap.env` 보기/편집 — 자격증명, RFC 백엔드, 블록리스트 정책, 활성 모듈. 시크릿 마스킹.

## MCP ABAP ADT 서버 — 고유 기능

prism은 **[abap-mcp-adt-powerup](https://github.com/abap-mcp-adt-powerup)** (150+ 툴)로 구동됩니다. 일반적인 Class / Program / Table / CDS / FM CRUD 외에도, 대부분의 MCP 서버가 다루지 않는 **classic Dynpro 아티팩트에 대한 완전 R/U/C 커버리지**를 추가:
...
prism의 규칙 코퍼스는 방대함 — 25+ `common/*.md` + 14 `configs/{MODULE}/*.md` + 30+ 산업/국가 파일. 모든 agent dispatch마다 전체 파일을 로드하는 건 토큰 낭비 + 모델 주의력 희석. **4-tier 컨텍스트 로딩 모델**([`common/context-loading-protocol.md`](../common/context-loading-protocol.md) 정의)은 "항상 로드해야 하는 안전 가드레일"과 "역할별 기본 세트"와 "조건 트리거"와 "per-task 킷"을 분리합니다.
...
- `/prism:create-program`의 Opus 사용 비중: −50% (`model-routing-rule.md` 라우팅 매트릭스 기준).
- Reviewer MAJOR 발견 정확도: 향상 — §1-§12 각 버킷이 12개 규칙 동시 훑기 대신 해당 규칙만 컨텍스트에 둠.

## 응답 프리픽스 규약 (v0.5.2+)

모든 `/prism:*` skill 트리거 응답은 다음 한 줄 프리픽스로 시작하여, 사용자가 어느 모델이 작업 중이고 어떤 sub-agent가 디스패치됐는지 한눈에 파악할 수 있도록 함:
...
규약은 모든 `/prism:*` SKILL.md의 `<Response_Prefix>` 블록이 [`common/model-routing-rule.md`](../common/model-routing-rule.md) § *Response Prefix Convention*를 참조하여 강제. 프리픽스는 skill 트리거된 턴에만 적용되며, 무관한 주제로 전환하는 사용자 메시지는 해당 턴부터 프리픽스가 제거됨.
...
`/prism:setup` (Step 4) 또는 `/prism:sap-option modules`로 설정. `create-program`, `create-object`, `analyze-cbo-obj`, 모든 컨설턴트 에이전트가 소비.

## SAP 플랫폼 인식 (ECC / S4 On-Prem / Cloud)

`prism:create-program`은 필수 SAP 버전 Preflight 실행. `.prism/config.json`의 `sapVersion`과 `abapRelease` 읽기:
...
`/prism:setup spro`는 고객별 SPRO 커스터마이징을 `.prism/spro-config.json`으로 추출. 컨설턴트는 `common/spro-lookup.md` 우선순위 따름:
...
**프로필** (`/prism:setup`에서 선택): `strict` / `standard` / `minimal` / `custom`. 사이트별 추가는 `.prism/blocklist-extend.txt`.

**설치** (`/prism:setup`이 자동, 수동):
```bash
...
| `gateway` | prism-rfc-gateway 미들웨어로 HTTPS | 10+ 팀, 중앙 배포 |
| 🆕 `zrfc` | HTTPS ICF 핸들러 `/sap/bc/rest/zmcp_rfc` | SOAP 차단 + OData Gateway 어려움 (전형적 ECC). SDK·Gateway 불필요 — 클래스 + SICF 노드 하나 |

`/prism:sap-option`으로 언제든 전환, MCP 재연결, `/prism:sap-doctor`로 검증.
